...
function renderNetworkCoverage(){
  ...
  const candidates=searches.flatMap(s=>s.candidates||[]),contacted=candidates.filter(c=>c.status!=='new').length,usable=candidates.filter(c=>['available','reservable','call','reserving','deposit_required','booked'].includes(c.status)).length,waiting=candidates.filter(c=>['awaiting','reserving'].includes(c.status)).length;
  return `<div class="network-overview network-compact"><span><b>Route bis zum Festival</b><small>${searches.length} Korridore · Auswahl oben</small></span><span class="network-count">${contacted} kontaktiert · ${usable} Optionen${waiting?' · '+waiting+' offen':''}</span></div>`;
}
function renderSleep(){
  ...
  const counts={reservable:0,call:0,waiting:0}; if(s)s.candidates.forEach(c=>{if(['available','reservable','deposit_required'].includes(c.status))counts.reservable++;else if(c.status==='call')counts.call++;else if(['awaiting','reserving'].includes(c.status))counts.waiting++;});
  const coverage=counts.reservable+counts.call, coverageTarget=s?.coverageTarget||3;
  ...
  ${s?`${s.candidates.filter(c=>c.status==='booked').map(c=>`<div class="sleep-secured"><div class="sleep-secured-label">Unterkunft gesichert</div>${sleepCandidateCard(s,c)}</div>`).join('')}<div class="sleep-summary"><div><b>${counts.reservable}</b><span>reservierbar</span></div><div><b>${counts.call}</b><span>spontan</span></div><div><b>${counts.waiting}</b><span>Antwort offen</span></div></div><div class="sleep-coverage${coverage>=coverageTarget?' good':''}">${coverage>=coverageTarget?'✓ Gute Auswahl für diesen Korridor':`Noch ${Math.max(0,coverageTarget-coverage)} passende Option${coverageTarget-coverage===1?'':'en'} für eine entspannte Auswahl ergänzen`}</div>
  <div class="sleep-viewbar"><div class="sleep-nav">${[['action','Optionen'],['waiting','Anfragen'],['closed','Absagen']].map(([v,l])=>`<button class="${sleepFilter===v?'active':''}" onclick="setSleepFilter('${v}')">${l}</button>`).join('')}</div><div class="sleep-segment"><button class="${sleepView==='list'?'active':''}" onclick="setSleepView('list')">Liste</button><button class="${sleepView==='map'?'active':''}" onclick="setSleepView('map')">Karte</button></div></div>${sleepView==='map'?`<div class="sleep-scope"><button class="${sleepMapScope==='night'?'active':''}" onclick="setSleepMapScope('night')">Aktueller Korridor</button><button class="${sleepMapScope==='route'?'active':''}" onclick="setSleepMapScope('route')">Ganze Route</button></div>`:''}
