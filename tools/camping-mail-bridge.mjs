...
async function scan(){
  ...
  const candidates=[];(state.sleepSearches||[]).forEach(search=>(search.candidates||[]).forEach(c=>{const p=(state.sleepPlaces||[]).find(x=>x.id===c.placeId)||{};candidates.push({searchId:search.id,candidateId:c.id,name:p.name||c.name,email:p.email||c.email,status:c.status,dateLabel:search.dateLabel});}));
...
async function applyEvents(encoded){
  ...
  return conditionalUpdate(state=>{const a=state.mailAssistant||(state.mailAssistant={processedMessageIds:[],draftRequests:[]});let changed=events.length===0,hadError=false;for(const e of events){if(e.assistantError!==undefined){a.lastError=String(e.assistantError||'');hadError=!!a.lastError;changed=true;continue;}const {candidate}=locate(state,e.searchId,e.candidateId);if(!candidate)continue;['status','reply','nextAction','nextActionDate','price','finalPrice','deposit','bookingRef','arrivalWindow','mailMessageId','mailThreadSubject','repliedAt','draftState','confirmedAt'].forEach(k=>{if(e[k]!==undefined)candidate[k]=e[k];});if(e.requestId){const req=(a.draftRequests||[]).find(x=>x.id===e.requestId);if(req){if(e.requestStatus)req.status=e.requestStatus;if(e.readyAt)req.readyAt=e.readyAt;if(e.sentAt)req.sentAt=e.sentAt;if(e.error)req.error=e.error;}}if(e.messageId&&!a.processedMessageIds.includes(e.messageId))a.processedMessageIds.push(e.messageId);changed=true;}a.processedMessageIds=a.processedMessageIds.slice(-200);a.lastRunAt=new Date().toISOString();a.nextRunAt=nextRunStamp();if(!hadError){a.lastSuccessAt=a.lastRunAt;a.lastError='';}return changed;});
}
