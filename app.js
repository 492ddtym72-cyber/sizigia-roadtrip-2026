...
function defaultState(){
  ...
  return {
    ...
    meta: { lastSaved: null, created: new Date().toISOString() },
    crew,
...
const SLEEP_PLACE_KEYS=['name','region','email','phone','link','lat','lng','notes'];
...
function sleepCandidateFields(raw={}){ const c=sleepCandidateView(raw), stored=c.lat!=null?{lat:+c.lat,lng:+c.lng}:null, auto=stored?null:pointOf({},c.name+' '+c.region); return [
  ...
  {key:'email',label:'E-Mail',value:c.email||''},{key:'phone',label:'Telefon',value:c.phone||''},{key:'link',label:'Karten-Link',value:c.link||''},
  {key:'pos',label:'Position auf der Karte',type:'map',value:stored,auto},
...
function syncSleepCandidate(s,c){
  let r=state.reminders.find(x=>x.id===c.reminderId);
  const reminderTitle=()=> c.status==='available'?`${c.name} reservieren · ${s.dateLabel}`:c.status==='draft_requested'?`${c.name}: E-Mail-Entwurf prüfen · ${s.dateLabel}`:c.status==='reserving'?`${c.name}: Reservierungsbestätigung abwarten · ${s.dateLabel}`:c.status==='deposit_required'?`${c.name}: Anzahlung prüfen · ${s.dateLabel}`:c.status==='followup'?`${c.name} erneut mailen${c.nextActionDate?' · '+c.nextActionDate:''}`:`${c.name} ${s.dateLabel}${c.status==='booked'?' ✅ bestätigt':c.status==='unavailable'?' ❌':''}`;
  if(['awaiting','available','draft_requested','reserving','deposit_required','followup','booked','unavailable'].includes(c.status)){
    if(!r){r={id:uid(),title:reminderTitle(),done:false,priority:c.status==='available',createdAt:new Date().toISOString(),createdBy:whoami()||null};state.reminders.unshift(r);c.reminderId=r.id;}
    r.title=reminderTitle(); r.done=['booked','unavailable'].includes(c.status); r.priority=c.status==='available';
  }
  if(c.status==='call'){
    if(r)r.done=true;
...
function sleepMailSubject(s,c,mode='inquiry'){if(c.mailThreadSubject)return c.mailThreadSubject.replace(/^(?:re|r|aw):\s*/i,'Re: ');return mode==='reserve'?`Reservation request – ${s.dateLabel||''} – ${c.name}`:mode==='followup'?`Follow-up: Pitch availability ${s.dateLabel||''}`:`Pitch availability ${s.dateLabel||''} – ${c.name}`;}
...
function sleepStatusSummary(c){if(c.reply)return c.reply;return c.status==='available'?'Der Campingplatz hat für diese Nacht eine reservierbare Option angeboten.':c.status==='call'?'Keine feste Reservierung, aber eine spontane Anfrage am Reisetag ist ausdrücklich möglich.':c.status==='reserving'?'Unsere Reservierungsanfrage wurde versendet; die definitive Bestätigung steht noch aus.':c.status==='deposit_required'?'Der Platz ist möglich, für die Buchung ist noch eine Anzahlung erforderlich.':c.status==='followup'?'Aktuell kein fester Platz; der Campingplatz empfiehlt, kurz vor der Reise erneut nachzufragen.':c.status==='awaiting'?'Anfrage gesendet, noch keine Antwort erhalten.':c.status==='unavailable'?'Für diese Nacht wurde keine nutzbare Möglichkeit angeboten.':'Noch keine Antwort oder belastbare Zusage erfasst.';}
function sleepCandidateCard(s,raw){ const c=sleepCandidateView(raw), st=SLEEP_STATUSES[c.status], maps=c.link||'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(c.name+(c.region?', '+c.region:'')), mode=sleepActionMode(c), hasAnswer=c.reply||c.replyQuote||c.pitchNote||c.parking||c.notes||c.status, statusLabel=c.status==='draft_requested'&&c.draftState==='ready'?'Entwurf bereit':st.label,latestReq=[...(state.mailAssistant.draftRequests||[])].reverse().find(x=>x.candidateId===c.id&&['opened','requested','ready','fallback'].includes(x.status));let actions='';if(c.status==='call'&&c.phone)actions+=`<a class="btn primary small" href="tel:${esc(c.phone.replace(/[^\d+]/g,''))}">📞 Jetzt anrufen</a>`;if(c.email&&!['booked','unavailable'].includes(c.status))actions+=`<button class="btn ${c.status==='call'?'ghost':'primary'} small" onclick="prepareSleepReply('${s.id}','${c.id}','${mode}')">✉️ ${c.status==='reserving'?'Nachricht vorbereiten':mode==='dimensions'?'Camperdaten vorbereiten':mode==='followup'?'Erneut fragen':mode==='reserve'?'Antwort vorbereiten':mode==='missing'?'Details nachfragen':mode==='deposit'?'Anzahlung klären':mode==='clarify'?'Buchung klären':'Antwort vorbereiten'}</button>`;if(latestReq&&c.status!=='reserving')actions+=`<button class="btn ghost small" onclick="markSleepDraftSent('${s.id}','${c.id}')">Als gesendet markieren</button>`;actions+=`<a class="btn ghost small" href="${esc(maps)}" target="_blank" rel="noopener">Karte</a><button class="btn ghost small" onclick="editSleepCandidate('${s.id}','${c.id}')">Bearbeiten</button>`;return `<div class="sleep-card ${c.status}"><div class="sleep-head"><div class="sleep-head-main"><h3>${esc(c.name)}</h3><div class="sleep-sub">${esc(c.region||s.region||'Ort noch offen')}</div></div><span class="sleep-status ${c.status}">${esc(statusLabel)}</span></div>
...
function sleepVisible(c){return sleepFilter==='action'?['available','call','draft_requested','reserving','deposit_required'].includes(c.status):sleepFilter==='waiting'?['new','awaiting','followup'].includes(c.status):false;}
const SLEEP_MAP_COLORS={booked:'#8ea8ff',available:'#5fd4a8',call:'#ffb257',draft_requested:'#54c8ff',reserving:'#54c8ff',deposit_required:'#ffd76b',followup:'#ffd76b',awaiting:'#54c8ff',new:'#b6bfcc',unavailable:'#737b8d'};
function buildSleepMap(s,candidates){
  const operational=['available','call','draft_requested','reserving','deposit_required'];let rows=(s.candidates||[]).filter(c=>operational.includes(c.status)).map(c=>({search:s,c}));
  if(sleepMapScope==='route'){
    const rank={booked:0,deposit_required:1,reserving:2,draft_requested:3,available:4,call:5}, seen=new Map();
...
function renderSleep(){
  ...
  const priority={booked:0,deposit_required:1,reserving:2,draft_requested:3,available:4,call:5,followup:6,awaiting:7,new:8,unavailable:9}; let candidates=s?[...s.candidates].sort((a,b)=>(priority[a.status]??99)-(priority[b.status]??99)):[];
  candidates=candidates.filter(sleepVisible);
  const counts={available:0,call:0}; if(s)s.candidates.forEach(c=>{if(counts[c.status]!==undefined)counts[c.status]++;});
  const coverage=counts.available+counts.call, coverageTarget=3;
  ...
  ${searches.length?`<div class="sleep-searches">${searches.map(x=>`<button class="sleep-search-tab${x.id===s.id?' active':''}" onclick="activeSleepSearchId='${x.id}';renderSleep()"><b>${esc(x.title)}</b><span>${esc([x.dateLabel,x.region].filter(Boolean).join(' · '))}</span></button>`).join('')}</div>`:''}${renderMailAssistantStatus()}${renderMailReviewQueue(s)}
  ${s?`${s.candidates.filter(c=>c.status==='booked').map(c=>`<div class="sleep-secured"><div class="sleep-secured-label">Unterkunft gesichert</div>${sleepCandidateCard(s,c)}</div>`).join('')}<div class="sleep-summary"><div><b>${counts.available}</b><span>reservierbar</span></div><div><b>${counts.call}</b><span>spontan anrufen</span></div></div><div class="sleep-coverage${coverage>=coverageTarget?' good':''}">${coverage>=coverageTarget?'✓ Solides Sicherheitsnetz für diese Nacht':`Noch ${coverageTarget-coverage} echte Option${coverageTarget-coverage===1?'':'en'} für ein solides Sicherheitsnetz ergänzen`}</div>
