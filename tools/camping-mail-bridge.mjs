...
try{
  if(command==='self-test'){const sample={crew:Array(6),vehicles:[{id:'v-camper',lengthM:'6.4'}]},search={startDate:'2026-08-02',endDate:'2026-08-03',arrivalWindowStart:'2026-08-02',arrivalWindowEnd:'2026-08-03',region:'Liguria',mode:'network'},candidate={name:'Test Camping'},place={name:'Test Camping'},modes=['inquiry','network_policy','followup','reserve','call','dimensions','missing','deposit','clarify'],bodies=modes.map(mode=>sharedDraftBody(sample,search,candidate,place,mode));if(bodies.some(body=>body.includes('Frederic')||body.includes('roadtrip group')||!body.endsWith('Kind regards,\n\n')))throw new Error('Signatur-Test fehlgeschlagen');if(bodies[3].includes('I would like to ask if you have availability'))throw new Error('Reservierungsantwort wiederholt Verfügbarkeitsanfrage');if(!bodies[1].includes('route remains flexible')||!bodies[1].includes('one-night stay'))throw new Error('Routen-Vorlage fehlt');if(!bodies[0].includes('arriving on any day from')||!bodies[0].includes('only need one night'))throw new Error('Flexibles Anreisefenster fehlt');console.log(JSON.stringify({ok:true,templates:modes.length,blankSignature:true,reservationDoesNotRepeatInquiry:true,networkPolicy:true,flexibleWindow:true}));}
