...
function defaultState(){
  ...
  return {
    ...
    sleepPlaces:[],
    sleepSearches:[],
    mailAssistant:{processedMessageIds:[],draftRequests:[],reviewQueue:[],runnerMode:'local',runners:{local:{lastSuccessAt:null,lastRunAt:null,lastError:'',nextRunAt:null},cloud:{lastSuccessAt:null,lastRunAt:null,lastError:'',nextRunAt:null}},lease:null,lastSuccessAt:null,lastRunAt:null,lastError:'',nextRunAt:null},
    log:[],
    festival:[
...
const CAMPING_NETWORK_HUBS = [
  {id:'liguria',title:'Ligurien',region:'Finale Ligure–Sestri Levante',startDate:'2026-08-03',endDate:'2026-08-04',target:4,order:1},
  {id:'provence-east',title:'Provence Ost',region:'Fréjus/Estérel · Verdon',startDate:'2026-08-04',endDate:'2026-08-05',target:4,order:2},
  {id:'cassis-marseille',title:'Cassis–Marseille',region:'Cassis · La Ciotat · Marseille',startDate:'2026-08-05',endDate:'2026-08-06',target:4,order:3},
  {id:'languedoc',title:'Languedoc',region:'Sète–Cap d’Agde',startDate:'2026-08-06',endDate:'2026-08-07',target:4,order:4},
  {id:'cote-vermeille',title:'Côte Vermeille',region:'Collioure–Argelès-sur-Mer',startDate:'2026-08-07',endDate:'2026-08-08',target:4,order:5},
  {id:'costa-brava',title:'Costa Brava Nord',region:'Cadaqués–Begur',startDate:'2026-08-08',endDate:'2026-08-09',target:4,order:6},
  {id:'huesca',title:'Huesca-Anfahrt',region:'Aínsa–Barbastro–Huesca',startDate:'2026-08-09',endDate:'2026-08-10',target:4,order:7}
];
...
function normalizeSleepCandidate(c){
  return Object.assign({id:uid(),name:'',region:'',email:'',phone:'',link:'',status:'new',price:'',tax:'',finalPrice:'',deposit:'',bookingRef:'',cancellationDeadline:'',arrivalWindow:'',confirmedAt:null,confirmation:{dates:false,party:false,camper:false,car:false},reply:'',replyQuote:'',pitchNote:'',parking:'',callWindow:'',nextAction:'',nextActionDate:'',notes:'',reminderId:null,contactId:null,contactedAt:null,repliedAt:null,mailMessageId:'',mailThreadSubject:'',draftState:'none'},c||{}, {status:SLEEP_STATUSES[c?.status]?c.status:'new',confirmation:Object.assign({dates:false,party:false,camper:false,car:false},c?.confirmation||{})});
}
...
function seedCampingSafetyNetwork(s){
  ...
  CAMPING_NETWORK_HUBS.forEach(hub=>{
    ...
    s.sleepSearches.push({id:uid(),title:hub.title,startDate:hub.startDate,endDate:hub.endDate,dateLabel:sleepDateLabelFromIso(hub.startDate,hub.endDate),region:hub.region,maxDrive:'flexibler Korridor',mode:'network',networkKey:hub.id,corridorOrder:hub.order,coverageTarget:hub.target,createdAt:new Date().toISOString(),candidates:[]});
  });
...
function migrate(s){
  ...
  s.sleepSearches = (s.sleepSearches||[]).map(x=>{const inferred=inferSleepDates(x.dateLabel);return Object.assign({id:uid(),title:'Schlafplatz-Suche',dateLabel:'',startDate:'',endDate:'',region:'',maxDrive:'',mode:'planned',createdAt:new Date().toISOString()},x,{startDate:x.startDate||inferred.startDate,endDate:x.endDate||inferred.endDate,candidates:(x.candidates||[]).map(normalizeSleepCandidate)});});
  ...
  (s.sleepSearches||[]).forEach(search=>(search.candidates||[]).forEach(c=>{if(c.status!=='draft_requested')return;const req=[...s.mailAssistant.draftRequests].reverse().find(x=>x.candidateId===c.id);c.status=req?.previousStatus||'available';c.draftState=req?.status||c.draftState||'none';}));
...
async function cloudPut(){
  const r = await fetch(CLOUD_URL, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(state)});
  if(!r.ok) throw new Error('HTTP ' + r.status);
...
async function syncNow(){
  ...
  setSyncStatus('syncing');
  try{
    const remote = await cloudGet(); // null bei leerer Datenbank
...
function addSleepSearch(today=false){
  ...
  openModal(today?'Suche für heute':'Schlafplatz-Suche hinzufügen',[
    {key:'title',label:'Titel',value:today?'Heute Abend':'Neue Nacht',placeholder:'z. B. Nacht bei Cassis'},
    {key:'startDate',label:'Anreise',type:'isoDate',value:start},{key:'endDate',label:'Abreise',type:'isoDate',value:end},
  ...
  ],v=>{ const undo=sleepUndo(), search={id:uid(),title:v.title.trim()||'Schlafplatz-Suche',startDate:v.startDate,endDate:v.endDate,dateLabel:sleepDateLabelFromIso(v.startDate,v.endDate),region:v.region.trim(),maxDrive:v.maxDrive.trim(),mode:today?'today':'planned',createdAt:new Date().toISOString(),candidates:[]}; state.sleepSearches.push(search); activeSleepSearchId=search.id; logChange('hat Schlafplatz-Suche „'+search.title+'“ hinzugefügt',undo); });
}
function editSleepSearch(id){ const s=state.sleepSearches.find(x=>x.id===id); if(!s)return; openModal('Schlafplatz-Suche bearbeiten',[
  {key:'title',label:'Titel',value:s.title},{key:'startDate',label:'Anreise',type:'isoDate',value:s.startDate||''},{key:'endDate',label:'Abreise',type:'isoDate',value:s.endDate||''},{key:'region',label:'Region / Korridor',value:s.region||''},{key:'maxDrive',label:'Maximale Fahrt / Umweg',value:s.maxDrive||''}
...
function sleepCandidateFields(raw={}){ const c=sleepCandidateView(raw), stored=c.lat!=null?{lat:+c.lat,lng:+c.lng}:null, auto=stored?null:pointOf({},c.name+' '+c.region); return [
  ...
  {key:'email',label:'E-Mail',value:c.email||''},{key:'phone',label:'Telefon',value:c.phone||''},{key:'link',label:'Karten-Link',value:c.link||''},{key:'officialUrl',label:'Website',value:c.officialUrl||''},{key:'contactVerified',label:'Kontaktprüfung',type:'checkbox',value:c.contactVerified!==false,text:'Offizielle E-Mail-Adresse geprüft'},
  {key:'pos',label:'Position auf der Karte',type:'map',value:stored,auto},
  ...
  {key:'parking',label:'Kleinwagen / Parken',type:'textarea',value:c.parking||''},{key:'notes',label:'Weitere Notizen',type:'textarea',value:c.notes||''}
]; }
...
function sleepEmailText(s,c,mode='inquiry'){
  const dates=sleepDateEnglish(s), party=`${state.crew.length} adults travelling with one camper and one small car`, end=`Thank you very much.\n\nKind regards,\n\n`;
  ...
  if(mode==='network_policy')return `Dear ${c.name} team,\n\nWe expect to travel through ${s.region||'your region'} around ${sleepArrivalEnglish(s)}, but our route remains flexible. We are ${party}.\n\nCould you please let us know whether you accept one-night stays on touring pitches and whether advance reservations are generally possible?\n\nIf no reservation is possible, may we call on the same day or arrive without a reservation in case a pitch is free? Please also let us know the best phone number and reception hours, any minimum-stay or camper restrictions, and whether the small car can be parked at or near the pitch.\n\n${end}`;
  if(mode==='call'){const walkIn=/vorbeifahren|spontaneous arrival|walk.?in/i.test((c.nextAction||'')+' '+(c.reply||''));return walkIn?`Dear ${c.name} team,\n\nThank you for confirming that we may arrive spontaneously ${dates}. We understand that a pitch cannot be guaranteed in advance for ${party}.\n\nWe will contact you shortly before arrival to check the current situation.\n\n${end}`:`Dear ${c.name} team,\n\nThank you for confirming that we may call you on ${sleepArrivalEnglish(s)} to ask about last-minute availability ${dates} for ${party}.\n\nWe will contact you ${c.callWindow?'during your reception hours ('+c.callWindow+')':'on the day before travelling to you'}.\n\n${end}`;}
...
function sleepMailSubject(s,c,mode='inquiry'){if(c.mailThreadSubject)return c.mailThreadSubject.replace(/^(?:re|r|aw):\s*/i,'Re: ');return mode==='network_policy'?`One-night camping policy – ${s.region||''} – ${c.name}`:mode==='reserve'?`Reservation request – ${s.dateLabel||''} – ${c.name}`:mode==='followup'?`Follow-up: Pitch availability ${s.dateLabel||''}`:`Pitch availability ${s.dateLabel||''} – ${c.name}`;}
function sleepMailto(s,c,mode='inquiry'){return `mailto:${encodeURIComponent(c.email)}?subject=${encodeURIComponent(sleepMailSubject(s,c,mode))}&body=${encodeURIComponent(sleepEmailText(s,c,mode))}`;}
...
function cancelSleepDraft(searchId,candidateId){const {c}=findSleep(searchId,candidateId);if(!c)return;const undo=sleepUndo(),req=[...state.mailAssistant.draftRequests].reverse().find(x=>x.candidateId===candidateId&&['requested','ready','fallback'].includes(x.status));if(c.status==='draft_requested')c.status=req?.previousStatus||'available';c.draftState='none';if(req)req.status='cancelled';logChange('hat den E-Mail-Entwurf für „'+sleepCandidateView(c).name+'“ verworfen',undo);save();renderAll();}
...
function sleepStatusSummary(c){if(c.reply)return c.reply;return c.status==='available'?'Der Campingplatz hat für diese Nacht eine reservierbare Option angeboten.':c.status==='reservable'?'Ein-Nacht-Reservierungen sind grundsätzlich möglich; die konkrete Nacht ist noch nicht bestätigt.':c.status==='call'?'Keine feste Reservierung, aber eine spontane Anfrage am Reisetag ist ausdrücklich möglich.':c.status==='reserving'?'Unsere Reservierungsanfrage wurde versendet; die definitive Bestätigung steht noch aus.':c.status==='deposit_required'?'Der Platz ist möglich, für die Buchung ist noch eine Anzahlung erforderlich.':c.status==='followup'?'Aktuell kein fester Platz; der Campingplatz empfiehlt, kurz vor der Reise erneut nachzufragen.':c.status==='awaiting'?'Anfrage gesendet, noch keine Antwort erhalten.':c.status==='unavailable'?'Für diese Nacht wurde keine nutzbare Möglichkeit angeboten.':'Noch keine Antwort oder belastbare Zusage erfasst.';}
function sleepCandidateCard(s,raw){ const c=sleepCandidateView(raw), st=SLEEP_STATUSES[c.status], maps=c.link||'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(c.name+(c.region?', '+c.region:'')), mode=s.mode==='network'&&c.status==='new'?'network_policy':c.status==='reservable'?'followup':sleepActionMode(c), hasAnswer=c.reply||c.replyQuote||c.pitchNote||c.parking||c.notes||c.status, statusLabel=c.status==='draft_requested'&&c.draftState==='ready'?'Entwurf bereit':st.label,latestReq=[...(state.mailAssistant.draftRequests||[])].reverse().find(x=>x.candidateId===c.id&&['opened','requested','ready','fallback'].includes(x.status));let actions='';if(c.status==='call'&&c.phone)actions+=`<a class="btn primary small" href="tel:${esc(c.phone.replace(/[^\d+]/g,''))}">📞 Jetzt anrufen</a>`;if(c.contactVerified===false)actions+=`<button class="btn ghost small" disabled>Kontakt prüfen</button>`;if(c.email&&c.contactVerified!==false&&!['booked','unavailable'].includes(c.status))actions+=`<button class="btn ${c.status==='call'?'ghost':'primary'} small" onclick="prepareSleepReply('${s.id}','${c.id}','${mode}')">✉️ ${c.status==='reserving'?'Nachricht vorbereiten':mode==='network_policy'?'Regeln anfragen':c.status==='reservable'?'Datum anfragen':mode==='dimensions'?'Camperdaten vorbereiten':mode==='followup'?'Erneut fragen':mode==='reserve'?'Antwort vorbereiten':mode==='missing'?'Details nachfragen':mode==='deposit'?'Anzahlung klären':mode==='clarify'?'Buchung klären':'Antwort vorbereiten'}</button>`;if(latestReq&&c.status!=='reserving')actions+=`<button class="btn ghost small" onclick="markSleepDraftSent('${s.id}','${c.id}')">Als gesendet markieren</button>`;if(c.officialUrl)actions+=`<a class="btn ghost small" href="${esc(c.officialUrl)}" target="_blank" rel="noopener">Website</a>`;actions+=`<a class="btn ghost small" href="${esc(maps)}" target="_blank" rel="noopener">Karte</a><button class="btn ghost small" onclick="editSleepCandidate('${s.id}','${c.id}')">Bearbeiten</button>`;return `<div class="sleep-card ${c.status}"><div class="sleep-head"><div class="sleep-head-main"><h3>${esc(c.name)}</h3><div class="sleep-sub">${esc(c.region||s.region||'Ort noch offen')}</div></div><span class="sleep-status ${c.status}">${esc(statusLabel)}</span></div>
  <div class="sleep-facts">${(c.finalPrice||c.price)?`<span class="sleep-fact">💶 ${esc(c.finalPrice||c.price)}${c.finalPrice?' gesamt':''}</span>`:''}${!c.finalPrice&&c.tax?`<span class="sleep-fact">+ ${esc(c.tax)}</span>`:''}${c.deposit?`<span class="sleep-fact">Anzahlung ${esc(c.deposit)}</span>`:''}${c.bookingRef?`<span class="sleep-fact">Nr. ${esc(c.bookingRef)}</span>`:''}${c.arrivalWindow?`<span class="sleep-fact">Anreise ${esc(c.arrivalWindow)}</span>`:''}${c.callWindow?`<span class="sleep-fact">📞 ${esc(c.callWindow)}</span>`:''}</div>
  ${(c.nextAction||c.nextActionDate)?`<div class="sleep-next">${esc(c.nextAction||'Nachfassen')}${c.nextActionDate?' · '+esc(c.nextActionDate):''}</div>`:''}
  ${hasAnswer?`<div class="sleep-answer"><div class="sleep-answer-label">${c.reply||c.replyQuote||c.repliedAt?'Das hat der Platz geantwortet':'Aktueller Stand'}</div><div class="sleep-answer-text">${esc(sleepStatusSummary(c))}</div>${c.pitchNote?`<div class="sleep-answer-meta"><b>Stellplatz:</b> ${esc(c.pitchNote)}</div>`:''}${c.parking?`<div class="sleep-answer-meta"><b>Auto:</b> ${esc(c.parking)}</div>`:''}${c.replyQuote?`<blockquote class="sleep-answer-quote">„${esc(c.replyQuote.replace(/^[„“\"']+|[„“\"']+$/g,''))}“</blockquote>`:''}</div>`:''}
  <div class="sleep-actions">${actions}</div></div>`; }
...
function sleepVisible(c){return sleepFilter==='action'?['available','reservable','call','draft_requested','reserving','deposit_required'].includes(c.status):sleepFilter==='waiting'?['new','awaiting','followup'].includes(c.status):false;}
...
function buildSleepMap(s,candidates){
  const operational=['available','reservable','call','draft_requested','reserving','deposit_required'];let rows=(s.candidates||[]).filter(c=>operational.includes(c.status)).map(c=>({search:s,c}));
  if(sleepMapScope==='route'){
    const rank={booked:0,deposit_required:1,reserving:2,draft_requested:3,available:4,reservable:5,call:6}, seen=new Map();
  ...
  return `<div class="sleep-map">${baseMapSvg(inner,'sleep')}</div><div class="sleep-legend"><span><i style="background:#5fd4a8"></i>Datum bestätigt</span><span><i style="background:#74c9a5"></i>Reservierung möglich</span><span><i style="background:#ffb257"></i>spontan anrufen</span><span><i style="background:#54c8ff"></i>angefragt</span></div>${plotted.length<rows.length?`<p class="hint">${rows.length-plotted.length} echte Option${rows.length-plotted.length===1?'':'en'} noch ohne Kartenposition.</p>`:''}`;
}
...
function renderMailAssistantStatus(){const a=state.mailAssistant||{},active=mailRunnerState(),r=active.runner,pending=(a.draftRequests||[]).filter(x=>['requested','ready','fallback'].includes(x.status)).length,reviews=(a.reviewQueue||[]).filter(x=>x.status==='pending').length,missing=!camperProfile().lengthM,last=r.lastSuccessAt?new Date(r.lastSuccessAt).toLocaleString('de-DE',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'noch nie',next=nextMailAssistantRun(r).toLocaleString('de-DE',{weekday:'short',hour:'2-digit',minute:'2-digit'}),stale=r.lastSuccessAt&&Date.now()-Date.parse(r.lastSuccessAt)>90*60*1000,label=active.mode==='cloud'?'Cloud':active.mode==='shadow'?'Mac · Cloud-Test':'Mac';return `<div class="mail-assistant${r.lastError||stale?' error':''}"><span>✉️</span><div><b>Mail-Assistent · ${label}</b> · letzter Check ${esc(last)} · nächster Check ${esc(next)}${pending?' · '+pending+' Entwurf'+(pending===1?'':'e')+' offen':''}${reviews?' · '+reviews+' manuell prüfen':''}${missing?' · Camperlänge fehlt':''}${stale?'<br>Seit über 90 Minuten kein erfolgreicher Check.':''}${r.lastError?`<br>${esc(r.lastError)}`:''}</div></div>`;}
...
function copyMailReview(id){const q=(state.mailAssistant.reviewQueue||[]).find(x=>x.id===id);if(!q)return;const {s,c}=mailReviewCandidate(q),view=c?sleepCandidateView(c):null,txt=`Bitte ordne diese Campingplatz-Antwort ein. Wähle genau einen Status: reservierbar, spontan anrufen/ankommen, später nachfragen, Anzahlung nötig, bestätigt, nicht verfügbar oder unklar. Gib danach eine kurze deutsche Zusammenfassung und den nächsten sinnvollen Schritt.\n\nCampingplatz: ${view?.name||q.campsiteName||'Unbekannt'}\nZeitraum: ${s?.dateLabel||q.dateLabel||''}\nBetreff: ${q.subject||''}\nNeuester Antwortteil:\n${q.excerpt||''}`,done=()=>toast('Für Codex kopiert');if(navigator.clipboard?.writeText)navigator.clipboard.writeText(txt).then(done,()=>fallbackCopy(txt,done));else fallbackCopy(txt,done);}
...
function renderNetworkCoverage(){
  const searches=(state.sleepSearches||[]).filter(s=>s.mode==='network').sort((a,b)=>(a.corridorOrder||99)-(b.corridorOrder||99));
  if(!searches.length)return '';
  return `<div class="network-overview"><div class="section-label">Campingoptionen bis zum Festival</div>${searches.map(s=>{const total=(s.candidates||[]).length,contacted=(s.candidates||[]).filter(c=>c.status!=='new').length,usable=(s.candidates||[]).filter(c=>['available','reservable','call','reserving','deposit_required','booked'].includes(c.status)).length,target=s.coverageTarget||4;return `<button class="network-row${activeSleepSearchId===s.id?' active':''}" onclick="activeSleepSearchId='${s.id}';renderSleep()"><span><b>${esc(s.title)}</b><small>${esc(s.region||'')}</small></span><span class="network-count">${contacted}/${target} kontaktiert · ${usable} nutzbar</span></button>`;}).join('')}</div>`;
}
function renderSleep(){
  ...
  const priority={booked:0,deposit_required:1,reserving:2,draft_requested:3,available:4,reservable:5,call:6,followup:7,awaiting:8,new:9,unavailable:10}; let candidates=s?[...s.candidates].sort((a,b)=>(priority[a.status]??99)-(priority[b.status]??99)):[];
  candidates=candidates.filter(sleepVisible);
  const counts={available:0,reservable:0,call:0}; if(s)s.candidates.forEach(c=>{if(counts[c.status]!==undefined)counts[c.status]++;});
  const coverage=counts.available+counts.reservable+counts.call, coverageTarget=s?.coverageTarget||3;
  document.getElementById('page-sleep').innerHTML=sectionBackButton()+`<div class="card sleep-hero"><div class="sleep-head"><div class="sleep-head-main"><h2>${s?esc(s.title):'Schlafplätze'}</h2><div class="sleep-sub">${s?esc([s.dateLabel,s.region,s.maxDrive].filter(Boolean).join(' · ')):'Optionen für die nächste Nacht sammeln'}</div></div>${s?`<button class="btn ghost small" onclick="editSleepSearch('${s.id}')">✎</button>`:''}</div><div class="sleep-hero-actions"><button class="btn primary small" onclick="${s?`addSleepCandidate('${s.id}')`:'addSleepSearch(true)'}">${s?'+ Campingplatz':'Heute suchen'}</button><button class="btn ghost small" onclick="addSleepSearch(false)">+ Nacht</button></div></div>
  ${searches.length?`<div class="sleep-searches">${searches.map(x=>`<button class="sleep-search-tab${x.id===s.id?' active':''}" onclick="activeSleepSearchId='${x.id}';renderSleep()"><b>${esc(x.title)}</b><span>${esc([x.dateLabel,x.region].filter(Boolean).join(' · '))}</span></button>`).join('')}</div>`:''}${renderNetworkCoverage()}${renderMailAssistantStatus()}${renderMailReviewQueue(s)}
  ${s?`${s.candidates.filter(c=>c.status==='booked').map(c=>`<div class="sleep-secured"><div class="sleep-secured-label">Unterkunft gesichert</div>${sleepCandidateCard(s,c)}</div>`).join('')}<div class="sleep-summary"><div><b>${counts.available}</b><span>Datum bestätigt</span></div><div><b>${counts.reservable}</b><span>Reservierung möglich</span></div><div><b>${counts.call}</b><span>spontan anrufen</span></div></div><div class="sleep-coverage${coverage>=coverageTarget?' good':''}">${coverage>=coverageTarget?'✓ Gute Auswahl für diesen Korridor':`Noch ${Math.max(0,coverageTarget-coverage)} passende Option${coverageTarget-coverage===1?'':'en'} für eine entspannte Auswahl ergänzen`}</div>
  <div class="sleep-viewbar"><div class="sleep-nav">${[['action','Echte Optionen'],['waiting','Offene Anfragen']].map(([v,l])=>`<button class="${sleepFilter===v?'active':''}" onclick="setSleepFilter('${v}')">${l}</button>`).join('')}</div><div class="sleep-segment"><button class="${sleepView==='list'?'active':''}" onclick="setSleepView('list')">Liste</button><button class="${sleepView==='map'?'active':''}" onclick="setSleepView('map')">Karte</button></div></div>${sleepView==='map'?`<div class="sleep-scope"><button class="${sleepMapScope==='night'?'active':''}" onclick="setSleepMapScope('night')">Diese Nacht</button><button class="${sleepMapScope==='route'?'active':''}" onclick="setSleepMapScope('route')">Gesamte Route</button></div>`:''}
  ${sleepView==='map'?buildSleepMap(s,candidates):(candidates.length?candidates.map(c=>sleepCandidateCard(s,c)).join(''):'<div class="sleep-empty">Keine Optionen in diesem Filter.</div>')}`:'<div class="card sleep-empty">Lege die erste Nacht oder eine spontane Suche für heute an.</div>'}`;
}
// Camping-Kontakte: Notfall-Liste zum spontanen Anrufen, falls der geplante
