...
async function seedNetwork(){
  const hubs=appConstArray('CAMPING_NETWORK_HUBS'),seeds=appConstArray('CAMPING_NETWORK_CANDIDATES'),verified=appConstSet('CAMPING_NETWORK_VERIFIED'),snapshot=await getState(),backupUrl=new URL('../backups/firebase-pre-v12-france-2026-07-14.json',import.meta.url);
  ...
  const before=networkCounts(snapshot.state),result=await conditionalUpdate(state=>{
    ...
    state.meta=state.meta||{};if(state.meta.frenchCritAirResearch!=='2026-07-14'){state.meta.frenchCritAirResearch='2026-07-14';changed=true;}if(state.meta.frenchPreferredContactsVerified!=='2026-07-14'){state.meta.frenchPreferredContactsVerified='2026-07-14';changed=true;}if((Number(state.schemaVersion)||0)<12){state.schemaVersion=12;changed=true;}if(changed){state.log=Array.isArray(state.log)?state.log:[];state.log.push({id:`network-v12-${Date.now()}`,ts:new Date().toISOString(),who:'Codex',desc:added?`hat ${added} recherchierte Frankreich-Optionen ergänzt`:'hat sechs bevorzugte Frankreich-Kontakte geprüft',undo:null});state.log=state.log.slice(-150);}return changed;
  });
