...
function renderNetworkCoverage(){
  ...
  return `<div class="network-overview"><div class="section-label">Notfallnetz bis zum Festival</div>${searches.map(s=>{const total=(s.candidates||[]).length,contacted=(s.candidates||[]).filter(c=>c.status!=='new').length,usable=(s.candidates||[]).filter(c=>['available','reservable','call','reserving','deposit_required','booked'].includes(c.status)).length,target=s.coverageTarget||4;return `<button class="network-row${activeSleepSearchId===s.id?' active':''}" onclick="activeSleepSearchId='${s.id}';renderSleep()"><span><b>${esc(s.title)}</b><small>${esc(s.region||'')}</small></span><span class="network-count">${contacted}/${target} kontaktiert · ${usable} nutzbar</span></button>`;}).join('')}</div>`;
}
function renderSleep(){
  ...
  ${s?`${s.candidates.filter(c=>c.status==='booked').map(c=>`<div class="sleep-secured"><div class="sleep-secured-label">Unterkunft gesichert</div>${sleepCandidateCard(s,c)}</div>`).join('')}<div class="sleep-summary"><div><b>${counts.available}</b><span>Datum bestätigt</span></div><div><b>${counts.reservable}</b><span>Reservierung möglich</span></div><div><b>${counts.call}</b><span>spontan anrufen</span></div></div><div class="sleep-coverage${coverage>=coverageTarget?' good':''}">${coverage>=coverageTarget?'✓ Solides Sicherheitsnetz für diesen Korridor':`Noch ${Math.max(0,coverageTarget-coverage)} echte Option${coverageTarget-coverage===1?'':'en'} für ein solides Sicherheitsnetz ergänzen`}</div>
