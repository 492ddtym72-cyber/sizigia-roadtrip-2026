(() => {
  ...
  const STORAGE_KEY = 'roadtrip-game-leaderboard-v1';
  ...
  function localAdapter() {
    ...
    return {
      async load() {
        const map = read();
        return Object.values(map).sort((a,b) => (b.best||0) - (a.best||0));
      },
      async submit(entry) {
        const map = read();
  ...
  function normalizeRows(rows) {
    ...
    rows.forEach(row => {
      if (!row) return;
      const id = String(row.playerId ?? row.id ?? row.name ?? '').trim();
      if (!id) return;
      const score = Math.max(0, Math.floor(Number(row.best ?? row.score ?? 0) || 0));
      const name = String(row.name || id);
      const old = best.get(id);
      if (!old || score > old.best) best.set(id, {playerId:id,name,best:score});
    });
    return [...best.values()].sort((a,b)=>b.best-a.best || a.name.localeCompare(b.name));
  }

  function resolvePlayer(options) {
    const p = options?.player || {};
    const name = String(p.name || 'Roadtripper');
    return { id: String(p.id || p.playerId || name), name };
  }
    ...
    root.innerHTML = `
      ...
      <section class="rtg-panel" hidden>
        ...
        <div class="rtg-leaderboard">
          <div class="rtg-leaderboard-title">Leaderboard <span>Personal bests</span></div>
  ...
  function createGame(root) {
    ...
    let raf = 0, last = 0, state = 'idle', score = 0, time = 0;
    let pipes = [], particles = [], shake = 0, flash = 0, spawnTimer = .85;
    let rowsBefore = [];
    const bird = {x:92,y:350,vy:0,rot:0,r:22,wing:0};
    const images = DEFAULT_BACKGROUNDS.map(src => { const i = new Image(); i.decoding='async'; i.src=src; return i; });
    ...
    function makePipeTexture() {
      ...
      for(let y=18;y<700;y+=56){
        for(let x=12;x<220;x+=44){
          pt.save();pt.translate(x,y);pt.rotate(Math.PI/4);pt.fillRect(-13,-13,26,26);pt.restore();
        }
      }
      pt.lineWidth=5;
      for(let y=12;y<700;y+=34){
        pt.strokeStyle=(Math.floor(y/34)%2)?'#ffd84a':'#3fe3c3';
        pt.beginPath();
        for(let x=0;x<=220;x+=11) pt.lineTo(x,y+Math.sin((x+y)/17)*9);
        pt.stroke();
      }
    ...
    function resize() {
      ...
      W = Math.max(320, r.width || innerWidth); H = Math.max(540, r.height || innerHeight);
    ...
    function reset() {
      ...
      state='idle';scoreEl.textContent='0';scoreEl.hidden=false;startEl.hidden=false;panel.hidden=true;
      root.classList.remove('rtg-paused');last=performance.now();
    }

    async function renderLeaderboard(rows) {
      const normalized=normalizeRows(rows);
      board.innerHTML = normalized.length ? normalized.slice(0,10).map((r,i)=>`<div class="rtg-board-row${r.playerId===runtime.player.id?' me':''}"><span class="rtg-rank">${i+1}</span><span class="rtg-board-name">${escapeHtml(r.name)}</span><span class="rtg-board-score">${r.best}</span></div>`).join('') : '<div class="rtg-empty">No scores yet.</div>';
    }

    async function loadRows() {
      try { rowsBefore=normalizeRows(await runtime.adapter.load()); }
      catch (_) { rowsBefore=[]; }
      return rowsBefore;
    }
    ...
    async function finish() {
      ...
      const oldBest = rowsBefore.find(r=>r.playerId===runtime.player.id)?.best || 0;
      let rows=rowsBefore;
      try { rows=normalizeRows(await runtime.adapter.submit({playerId:runtime.player.id,name:runtime.player.name,score})); } catch (_) {}
      const nowBest = rows.find(r=>r.playerId===runtime.player.id)?.best || Math.max(oldBest,score);
      const newBest = score>oldBest && score===nowBest;
      runScore.textContent=String(score);
      panelTitle.textContent=newBest?'PERSONAL BEST':'RUN OVER';
      panelTitle.classList.toggle('best',newBest);
    ...
    function difficulty() {
      const t=1-Math.exp(-score/34);
      return { speed:W*(.335+.13*t), gap:H*(.305-.072*t), interval:1.48-.25*t };
    }

    function spawnPipe() {
      const d=difficulty(), margin=Math.max(98,H*.115), min=margin+d.gap/2, max=H-margin-d.gap/2;
      const previous=pipes.length?pipes[pipes.length-1].gapY:H*.5;
      const maxDelta=H*.19;
      let center=min+Math.random()*Math.max(1,max-min);
      center=Math.max(min,Math.min(max,previous+Math.max(-maxDelta,Math.min(maxDelta,center-previous))));
      pipes.push({x:W+35,w:Math.max(72,W*.19),gapY:center,gap:d.gap,scored:false});
    }

    function milestone() {
      if(score===0 || score%10!==0) return;
      milestoneEl.textContent=`FLOW ${score}`;milestoneEl.classList.remove('show');void milestoneEl.offsetWidth;milestoneEl.classList.add('show');
      flash=.42;
    ...
    function hit(p) {
      const br=bird.r*.68;
      if(bird.x+br<p.x||bird.x-br>p.x+p.w) return false;
      return bird.y-br<p.gapY-p.gap/2 || bird.y+br>p.gapY+p.gap/2;
    }
    ...
    function update(dt) {
      ...
      const d=difficulty();
      bird.vy+=Math.max(930,H*1.12)*dt;bird.y+=bird.vy*dt;bird.rot=Math.min(1.12,bird.rot+dt*2.45);
      ...
      if(bird.y-bird.r<0||bird.y+bird.r>H) finish();
    ...
    function coverImage(img, alpha, drift) {
      ...
      const maxX=Math.max(0,img.naturalWidth-sw), maxY=Math.max(0,img.naturalHeight-sh);
      const sx=maxX*.5 + Math.sin(time*.018+drift)*Math.min(maxX*.15,16);
      const sy=Math.max(0,maxY*(.32+drift*.08));
      ctx.globalAlpha=alpha;ctx.drawImage(img,sx,sy,sw,sh,0,0,W,H);ctx.globalAlpha=1;return true;
    }

    function drawBackground() {
      ctx.fillStyle='#24104d';ctx.fillRect(0,0,W,H);
      const index=Math.floor(score/12)%images.length, next=(index+1)%images.length, mix=(score%12)/12;
      const ok=coverImage(images[index],1,index);
      if(ok&&mix>.72) coverImage(images[next],Math.min(1,(mix-.72)/.28),next);
      if(!ok){const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,'#29095e');g.addColorStop(.45,'#d94878');g.addColorStop(1,'#122a3c');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);}
      ctx.fillStyle='rgba(20,6,42,.11)';ctx.fillRect(0,0,W,H);
      const ex=W*.54,ey=H*.26,er=Math.min(W,H)*.095;
      const halo=ctx.createRadialGradient(ex,ey,er*.7,ex,ey,er*1.8);halo.addColorStop(0,'rgba(255,185,66,.65)');halo.addColorStop(1,'rgba(255,98,45,0)');ctx.fillStyle=halo;ctx.beginPath();ctx.arc(ex,ey,er*1.8,0,Math.PI*2);ctx.fill();
    ...
    function drawPipe(p,top) {
      const endTop=p.gapY-p.gap/2, startBottom=p.gapY+p.gap/2;
      const y=top?0:startBottom, h=top?endTop:H-startBottom;if(h<=0)return;
      ...
      const capH=30,cy=top?endTop-capH:startBottom;
      const cg=ctx.createLinearGradient(p.x,cy,p.x+p.w,cy+capH);cg.addColorStop(0,'#f2aa32');cg.addColorStop(.5,'#d65079');cg.addColorStop(1,'#2fb597');
      ctx.fillStyle=cg;ctx.beginPath();ctx.roundRect(p.x-7,cy,p.w+14,capH,9);ctx.fill();ctx.strokeStyle='#171126';ctx.lineWidth=5;ctx.stroke();
      ctx.strokeStyle='#ffe25a';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(p.x,cy+8);ctx.lineTo(p.x+p.w,cy+21);ctx.stroke();
    }
    ...
    function drawBird() {
      ...
      ctx.shadowBlur=0;ctx.fillStyle='#49215e';ctx.beginPath();ctx.ellipse(0,0,30,25,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#ffb63c';ctx.lineWidth=3;ctx.stroke();
      ctx.fillStyle='#8e3db0';ctx.beginPath();ctx.ellipse(-10,9,20,12,.35,0,Math.PI*2);ctx.fill();
      const wingLift=bird.wing*7;ctx.fillStyle='#d34a9e';ctx.beginPath();ctx.ellipse(-16,11-wingLift,22,9,-.5,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#2bd1bd';ctx.beginPath();ctx.arc(8,-7,14,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#ffe06a';ctx.lineWidth=4;ctx.stroke();ctx.fillStyle='#121222';ctx.beginPath();ctx.arc(9,-7,8,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(12,-10,2.5,0,Math.PI*2);ctx.fill();
      ctx.strokeStyle='#ff4dae';ctx.lineWidth=3;ctx.beginPath();ctx.arc(7,-7,19,-2.2,2.2);ctx.stroke();
      ctx.fillStyle='#ffb13c';ctx.beginPath();ctx.moveTo(22,-3);ctx.lineTo(45,5);ctx.lineTo(22,13);ctx.closePath();ctx.fill();ctx.strokeStyle='#21142d';ctx.lineWidth=2;ctx.stroke();
      ctx.fillStyle='#ffd447';ctx.beginPath();ctx.arc(-14,-14,3,0,Math.PI*2);ctx.fill();ctx.fillStyle='#3ee4c9';ctx.beginPath();ctx.arc(-20,-5,3,0,Math.PI*2);ctx.fill();
      ctx.restore();
    }
    ...
    function draw() {
      ...
      drawBackground();pipes.forEach(p=>{drawPipe(p,true);drawPipe(p,false);});drawParticles();drawBird();
      if(state==='idle'){ctx.fillStyle='rgba(15,4,35,.1)';ctx.fillRect(0,0,W,H);}if(flash>0){ctx.fillStyle=`rgba(112,255,203,${flash*.3})`;ctx.fillRect(0,0,W,H);}ctx.restore();
    }
    ...
    async function open(options={}) {
      runtime.options=options;runtime.player=resolvePlayer(options);runtime.adapter=options.leaderboardAdapter||runtime.adapter||localAdapter();
      if(Array.isArray(options.backgrounds)&&options.backgrounds.length){images.splice(0,images.length,...options.backgrounds.map(src=>{const i=new Image();i.decoding='async';i.src=src;return i;}));}
      ...
      root.dispatchEvent(new CustomEvent('roadtripgameopen',{detail:{player:runtime.player}}));
    }
