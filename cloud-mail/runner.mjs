...
async function releaseLease(error=''){await updateState(state=>{const a=assistant(state),r=a.runners.cloud;a.runnerMode=mode;r.lastRunAt=nowIso();r.nextRunAt=nextRun();if(error)r.lastError=error;else{r.lastSuccessAt=r.lastRunAt;r.lastError='';}if(a.lease?.owner===OWNER)a.lease=null;return true;});}
...
async function main(){if(!['shadow','cloud'].includes(mode)){console.log(JSON.stringify({ok:true,disabled:true}));return;}for(const key of ['ICLOUD_EMAIL','ICLOUD_APP_PASSWORD'])if(!process.env[key])throw new Error(`${key} fehlt`);await acquireLease();let client,error='';try{const {state}=await readState();client=await connectIcloud();const boxes=await client.list(),inbox=chooseMailbox(boxes,'\\Inbox',/^inbox$/i)||'INBOX',sent=chooseMailbox(boxes,'\\Sent',/sent|gesendet/i),drafts=chooseMailbox(boxes,'\\Drafts',/draft|entwurf/i);const inbound=await collectInbox(client,inbox,state),draftEvents=await createDrafts(client,drafts,state),sentEvents=await detectSent(client,sent,state);await applyEvents([...inbound,...draftEvents,...sentEvents]);console.log(JSON.stringify({ok:true,mode,matched:inbound.length,drafts:draftEvents.length,sent:sentEvents.length}));}catch(e){error=String(e.message||e);throw e;}finally{if(client)await client.logout().catch(()=>{});await releaseLease(error).catch(()=>{});}}
